...
public class EffectBeatGrid : MonoBehaviour
{
    ...
    private void UpdateLines()
    {
        ...
        if (_bootstrap.EditorBootstrap == null) return;
        var cam = _bootstrap.EditorBootstrap.EditorCamera;
        if (cam == null) return;
        ...
        float panelCenterX = _bootstrap.PanelCenterX;
        ...
        for (float b = startBeat; b <= maxBeat + 0.0001f; b += step)
        {
            ...
            PlaceLine(idx, panelCenterX, worldY, panelWidth, color);
            idx++;
        }
    ...
    private void EnsureLine(int idx)
    {
        while (_pool.Count <= idx)
        {
            ...
            go.name = "EffectBeatLine_" + _pool.Count;
